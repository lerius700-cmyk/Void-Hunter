"""Stage 2 (BLOQUE 64.B): Post-process AI bases for GOLIATH.

For each state:
  1. Load the 1024x1024 base PNG.
  2. Crop bottom 15% (removes the AI watermark).
  3. Center-crop to square.
  4. Resize to 96x80 with LANCZOS.
  5. Map pixels to the 64-color palette.
  6. Run the shared damero-cleaner with distance_threshold=90.
  7. Threshold alpha to binary.
  8. Save to Assets/sprites/redesign/goliath/<state>/_source.png.

BLOQUE 64.B deviation: the prompt now requests BORDERLESS + TRANSPARENT
background, so the existing transparentize-dameru postprocess is a
safety net. We pass the AI image through unchanged except for
crop/resize/palette.

Usage:
    python tools/redesign_bosses/02b_postprocess_64b.py
    python tools/redesign_bosses/02b_postprocess_64b.py --state phase1_idle
"""
...
_transparentize_background = _ship_pp._transparentize_background
...
def make_source_png(base: Image.Image, out_path: Path) -> None:
    """Full pipeline: square -> pre-transp -> resize 96x80 -> post-transp -> palette -> save.

    BLOQUE 64.B: the AI prompt requests BORDERLESS + TRANSPARENT but the
    generator still leaves a faint gray border around the silhouette.
    We apply _transparentize_background BEFORE the resize (catches the
    full-resolution black/white checker) AND _transparentize_damero_after_resize
    AFTER the resize (catches the mid-gray blend left by LANCZOS).
    """
    square = preprocess_base(base)
    square = _transparentize_background(square)
...
def postprocess_one(state_key: str, *, force: bool = False) -> None:
...
def main() -> int:
    ...
    parser.add_argument("--force", action="store_true", help="Overwrite existing _source.png")
