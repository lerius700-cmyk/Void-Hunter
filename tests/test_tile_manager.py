...
def test_tile_manager_tiles_not_rotated():
    """Tiles stay in their original AI-generated orientation (no rotation)."""
    from src.systems.tile_manager import TileManager
    ...
    # Without rotation, pixel(x, y) in surface should equal pixel(x, y) in source
    raw = Image.open(tiles_dir / "tile_01_atmosphere_exit.png")
    src_pixel = raw.getpixel((0, 0))
    loaded_pixel = tm._tile_surfaces[0].get_at((0, 0))[:3]
    assert src_pixel == loaded_pixel, f"Expected {src_pixel}, got {loaded_pixel}"
